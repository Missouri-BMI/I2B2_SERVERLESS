...
val configMap: Map[String, String] = Map( "shrine.shrineHubBaseUrll" -> "https://shrine-dev1.catalyst:6443")
...
ConfigSource.atomicConfig.configForBlock(configMap, "MessageQueueClientDev1") {
  ...
  val numberOfQEPs: Int = 24 // can do 24 (with 5 messages), but not 29, with the netty client. spray client limit was > 62
  val numberOfMessages: Int = 8 //can do 5 messages with 24 QEPs, but not 8
...
//    System.out.println(s"Deleted queue: $queueName, $delete")
    ...
    for (i <- 1 to numberOfMessages) {
      messageQueueWebClient.sendIO(s"Message$i sent to $queueName", queue).unsafeRunSync()
