...
object NetworkMetrics {
  ...
  private val NODES_COMMAND = "nodes"
  ...
  private val parser: OParser[Unit, CommandLine] = {
    ...
    OParser.sequence(
      builder.programName("networkMetrics"),
      builder.head("Gather network-wide metrics of SHRINE"),
      ...
      builder.cmd(NODES_COMMAND)
        .action((_, c) => c.copy(command = NODES_COMMAND))
        .text("Create a csv file of nodes"),
  ...
  def doCommand(args: Array[String]): Unit = {
    OParser.parse(parser, args, CommandLine()) match {
      case Some(commandLine: CommandLine) =>
        ...
        val passwordConfigPath = "shrine.hub.database.dataSourceConfig.credentials.password"
        ...
        ConfigSource.configForBlock(passwordConfigPath, SealerRevealer.reveal(databasePassword), "password") {
          commandLine.command match {
            ...
            case command if command == NODES_COMMAND => NodeCsv.writeNodeCsv(outFile, startDate, endDate, commandLine.researcher)
          }
        }
