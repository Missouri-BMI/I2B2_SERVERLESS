...
update SHRINE_QUERY SET username=(@temp:=username), username = domain, domain = @temp where username not in (select distinct(userName) from adapterAuditDB.queriesReceived);
